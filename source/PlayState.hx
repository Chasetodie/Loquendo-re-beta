			...
			if(allowedToHeadbang)
			{
				...
				case 'Hack':
					...
					if(curBeat < 250)
						{
								// Beats to skip or to stop GF from cheering
						if(curBeat != 40 && curBeat != 60)
							{
								new FlxTimer().start(1, function(tmr:FlxTimer)
								{
									var spacebar:FlxSprite = new FlxSprite(-100, 100).loadGraphic(Paths.image('logoBumpin'));
									spacebar.antialiasing = true;
									spacebar.animation.addByPrefix('space', 'logo bumpin', 24);
									spacebar.animation.play('space');
									spacebar.setGraphicSize(Std.int(spacebar.width * 1.1));
...
//						if (curBeat % 40 == 60 && SONG.song == 'Hack' && dad.curCharacter == 'loqangry' && boyfriend.curCharacter == 'bf') //curbeat 17//
//							{
//								//new FlxTimer().start(1, function(tmr:FlxTimer)
//								//{
//									var spacebar:FlxSprite = new FlxSprite(-100, 100).loadGraphic(Paths.image('logoBumpin'));
